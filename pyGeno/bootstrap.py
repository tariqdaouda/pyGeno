...
def printRemoteDatawraps(location=conf.pyGeno_REMOTE_LOCATION):
    ...
    a datawraps.json in the following format:
...
def importRemoteGenome(name, batchSize=100):
    ...
    try:
        ...
        raise AttributeError(
            "There's no remote genome datawrap by the name of: '%s'" % name)
...
def importGenome(name, batchSize=100):
    """Import a genome shipped with pyGeno. Most of the datawraps only contain
    URLs towards data provided by third parties."""
    path = os.path.join(this_dir, "bootstrap_data", "genomes/" + name)
    PG.importGenome(path, batchSize)


def importSNPs(name):
    """Import a SNP set shipped with pyGeno. Most of the datawraps only contain
    URLs towards data provided by third parties."""
