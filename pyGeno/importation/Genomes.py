...
def _getFile(fil, directory):
    if fil.startswith(("https://", "http://", "ftp://")):
...
def importGenome(packageFile, batchSize=50, verbose=0):
    ...
    if os.path.isdir(seqTargetDir):
        raise KeyError("The directory %s already exists, Please call deleteGenome() before reinstalling" % seqTargetDir)
            ...
            if transId is not None:
                ...
                if exonKey is not None:
                    ...
                    if regionType == 'exon':
                        ...
                        if store.exons[exonKey].end is None or end > store.exons[exonKey].end:
                        ...
                        if strand == '+':
                            if store.exons[exonKey].CDS_end is not None:
                        ...
                        if strand == '-':
                            if store.exons[exonKey].CDS_start is not None:
