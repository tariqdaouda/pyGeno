...
# Proxy for the raba database
db = None
# Proxy for the raba database configuration
dbConf = None


def version():
    """returns a tuple describing pyGeno's current version"""
    return (pyGeno_FACE, pyGeno_BRANCH, pyGeno_VERSION_NAME,
            pyGeno_VERSION_RELEASE_LEVEL, pyGeno_VERSION_NUMBER,
            pyGeno_VERSION_BUILD_TIME)
...
def getGenomeSequencePath(specie, name):
    return os.path.normpath(pyGeno_DATA_PATH + '/%s/%s' % (specie.lower(),
                                                           name))
...
def createDefaultConfigFile():
    ...
    s = "[pyGeno_config]\nsettings_dir=%s\nremote_location=%s" % (
        pyGeno_SETTINGS_DIR, pyGeno_REMOTE_LOCATION)
...
def removeFromDBRegistery(obj):
    """rabaDB keeps a record of loaded objects to ensure consistency between
    different queries. This function removes an object from the registry"""
    rabaDB.Raba.removeFromRegistery(obj)


def freeDBRegistery():
    """rabaDB keeps a record of loaded objects to ensure consistency between
    different queries. This function empties the registry"""
...
def pyGeno_init():
    ...
    if not checkPythonVersion():
        raise PythonVersionError(
            "==> FATAL: pyGeno only works with python 2.7 and above, please upgrade your python version"
        )
    ...
    pyGeno_RABA_DBFILE = os.path.normpath(
        os.path.join(pyGeno_SETTINGS_PATH, "pyGenoRaba.db"))
    pyGeno_DATA_PATH = os.path.normpath(
        os.path.join(pyGeno_SETTINGS_PATH, "data"))
    ...
    # launching the db
    rabaDB.rabaSetup.RabaConfiguration(pyGeno_RABA_NAMESPACE,
                                       pyGeno_RABA_DBFILE)
