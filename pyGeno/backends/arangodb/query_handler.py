...
class QueryHandler(QueryHandler_ABS):
    ...
    def lazy_query(self, anchor_type, anchor_id=None, fetch_type=None, **lazy_args):
        return self.dict_query(anchor_type, anchor_id, fetch_type, lazy_args)
    ...
    def dict_query(self, anchor_type, anchor_id=None, fetch_type=None, dct=None,):
        if fetch_type is None :
            return self._get_dict_result(anchor_type, dct)
        else:
            if anchor_id is None :
                raise ValueError("anchor_id must be supplied for a join")
            return self._get_dict_join_result(anchor_type, anchor_id, fetch_type, dct)
    ...
    def rich_query(self, anchor_type, anchor_id, fetch_type, pygeno_filter):
