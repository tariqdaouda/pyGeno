...
class ChrosomeSequence(object):
    """Represents a chromosome sequence. If 'refOnly' no ploymorphisms are
    applied and the ref sequence is always returned"""
        ...
        for start, setPolys in six.iteritems(polys):
            ...
            if sequenceModifier is not None:
                if sequenceModifier.__class__ is SF.SequenceDel:
                    ...
                    data[seqPos:(seqPos + sequenceModifier.length
                    ...
                    seqPos = seqPos + sequenceModifier.offset
                    data[seqPos] = "%s%s" % (data[seqPos],
                                             sequenceModifier.bases)
                else:
                    raise TypeError(
...
class Chromosome(pyGenoRabaObjectWrapper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        path = '%s/chromosome%s.dat' % (self.genome.getSequencePath(),
                                        self.number)
    ...
    def __str__(self):
        return "Chromosome: number %s > %s" % (self.wrapped_object.number,
                                               str(self.wrapped_object.genome))
