...
class Sequence_modifiers(object):
    ...
    def addSource(self, name, snp):
        """Optional, you can keep a dict that records the polymorphims that
        were mixed together to make self. They are stored into self.sources"""
    ...
    def __init__(self, bases, sources={}, ref='-'):
        ...
        if ref != '-':
            if ref == bases[:len(ref)]:
                ...
                # -1 because if the insertion are after the last nucleotide we
                # go out of table
                self.offset -= 1
            else:
                raise NotImplemented(
                    "This format of Insetion is not accepted. Please change your format, or implement your format in pyGeno."
                )
        ...
        if alt != '-':
            if ref is not None:
                if alt == ref[:len(alt)]:
                    ...
                    raise NotImplemented(
                        "This format of Deletion is not accepted. Please change your format, or implement your format in pyGeno."
                    )
            else:
                raise Exception(
                    "You need to add a ref sequence in your call of SequenceDel. Or implement your format in pyGeno."
                )
...
class DefaultSNPFilter(SNPFilter):
    ...
    This is also a template that you can use for own filters. A prototype for a
    custom filter might be:
    ...
    Where SNP_Set1 and SNP_Set2 are the actual names of the snp sets supplied
    to the genome. In the context of the function they represent single
    polymorphisms derived from thoses sets that occur at the same position.

    Whatever goes on into the function is absolutely up to you, but in the end,
    it must return an object of one of the following classes:
    ...
    def filter(self, chromosome, **kwargs):
        """The default filter mixes applied all SNPs and ignores insertions and
        deletions."""
        ...
        for snpSet, snp in six.iteritems(kwargs):
            ...
            if snp.alt[0] == '-':
                ...
                sources[snpSet] = snp
                # if not an indel append the polymorphism
        ...
        # optional: we keep a record of the polymorphisms that were used during
        # the process
