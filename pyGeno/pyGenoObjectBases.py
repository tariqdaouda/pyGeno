...
def nosave():
    raise ValueError(
        'You can only save object that are linked to reference genomes')


class pyGenoRabaObject(Raba):
    """pyGeno uses rabaDB to persistenly store data. Most persistent objects
    have classes that inherit from this one (Genome_Raba, Chromosome_Raba,
    Gene_Raba, Protein_Raba, Exon_Raba). Theses classes are not mean to be
    accessed directly. Users manipulate wrappers such as : Genome, Chromosome
    etc... pyGenoRabaObject extends the Raba class by adding a function _curate
    that is called just before saving. This class is to be considered abstract,
    ...
    def _curate(self):
        ...
        raise TypeError(
            "This method is abstract and should be implemented in child")
...
class RLWrapper(object):
    ...
    def __getitem__(self, i):
        return self.listObjectType(
            wrapped_object_and_bag=(self.rl[i], self.rabaObj.bagKey))
...
class pyGenoRabaObjectWrapper(object):
    ...
    def __init__(self, wrapped_object_and_bag=(), *args, **kwargs):
        ...
        if wrapped_object_and_bag != ():
            ...
            pyGenoRabaObjectWrapper._bags[self.bagKey][self._getObjBagKey(
                self.wrapped_object)] = self
        else:
    ...
    def _makeLoadQuery(self, objectType, *args, **coolArgs):
        ...
        f = RabaQuery(
            objectType._wrapped_class,
            namespace=self._wrapped_class._raba_namespace)
        # [:-5] removes _Raba from class name
        coolArgs[self._wrapped_class.__name__[:-5]] = self.wrapped_object
    ...
    def iterGet(self, objectType, *args, **coolArgs):
        """Same as get. But retuns the elements one by one, much more efficient
        for large outputs"""
    ...
    def __getattr__(self, name):
        ...
        if name == 'save' or name == 'delete':
            raise AttributeError(
                "You can't delete or save an object from wrapper, try .wrapped_object.delete()/save()"
            )
        ...
        if isRabaObject(attr):
            ...
            if attrKey in pyGenoRabaObjectWrapper._bags[self.bagKey]:
                ...
                wCls = pyGenoRabaObjectWrapper_metaclass._wrappers[
                    attr._rabaClass]
    ...
    def ensureGlobalIndex(cls, fields):
        ...
        list of fields for Multi-Column Indices or simply the name of a single
        field. A global index is an index on the entire type. A global index on
        'Transcript' on field 'name', will index the names for all the
    ...
    def getSequencesData(self):
        ...
        raise NotImplementedError(
            "This fct loads non binary sequences and should be implemented in child if needed"
        )
    ...
    def _load_bin_sequence(self):
        ...
        raise NotImplementedError(
            "This fct loads binary sequences and should be implemented in child if needed"
        )
